    ...
    >
      ...
      >
        ...
        <div style={{ color: C.muted, fontSize: 14, lineHeight: 1.6, marginBottom: 20 }}>
          Este construtor simplificado foi mantido só para o app não quebrar.
          Depois você pode trocar por uma versão mais completa.
        </div>
        ...
        >
          Dias configurados: {Object.keys(weekDiet || {}).length}
          <br />
          Medicamentos: {(meds || []).length}
        </div>

        <div style={{ display: "flex", gap: 10 }}>
          <Btn onClick={() => onSave(weekDiet, meds)} color={C.accent} full>
            Salvar
          </Btn>
          <button
            onClick={onClose}
...
export default function AlunoApp({ profile, onSignOut, onProfileUpdate }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setLoadError(e?.message || "Erro ao carregar seus dados.");
    } finally {
      setLoading(false);
    }
  }, [profile.id, profile.nutri_id, tk]);
  ...
  useEffect(() => {
    const check = () => {
      ...
      const hm = nowDate.getHours() * 60 + nowDate.getMinutes();

      meds.forEach(med =>
        med.times?.forEach(t => {
          const [h, m] = t.split(":").map(Number);
          if (Math.abs(hm - (h * 60 + m)) <= 5) setMedAlert(med);
        })
      );
    };
  ...
  const toggleFood = async (mealId, foodId, status) => {
    const meal = todayLog[mealId] || {};
    const cur = meal[foodId];
    const newLog = {
      ...todayLog,
      [mealId]: { ...meal, [foodId]: cur === status ? null : status }
    };
  ...
  const saveWeekDiet = async (wd, ms) => {
    setSaving(true);
    try {
      await DB.saveWeekDiet(profile.id, wd);
      await DB.saveMeds(profile.id, ms);
      setWeekDiet(wd);
      setMeds(ms);
    } finally {
      setSaving(false);
  ...
  const weightData = measurements
    .filter(m => m.peso)
    .map(m => ({ v: parseFloat(m.peso), label: m.date }));

  const waistData = measurements
    .filter(m => m.cintura)
    .map(m => ({ v: parseFloat(m.cintura), label: m.date }));
      ...
      {medAlert && (
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 20, fontWeight: 800, color: C.amber, marginBottom: 4 }}>
              Hora do remédio!
            </div>
            ...
            <Btn onClick={() => setMedAlert(null)} color={C.amber} full>
              Tomei ✓
            </Btn>
          </div>
        </div>
      ...
      {mealModal && (
        ...
        >
          ...
          >
            ...
            >
              <div>
                ...
                <div style={{ fontFamily: "'Lora',serif", fontSize: 24, fontWeight: 700 }}>
                  {mealModal.emoji} {mealModal.name}
                </div>
              </div>
              <button
            ...
            {(() => {
              const mm = meds.filter(m => m.times?.includes(mealModal.time));
              return mm.length > 0 ? (
                <div
                  style={{
                ...
                >
                  ...
                  {mm.map(m => (
                    ...
                    >
                      ...
                      <span style={{ color: C.amber }}>{m.dose}</span>
                    </div>
                  ))}
                </div>
              ) : null;
            ...
            {(mealModal.foods || []).map(f => {
              const st = getFoodStatus(mealModal.id, f.id);
              return (
                <div
                  key={f.id}
                  style={{
                    background:
                      st === "ok"
                        ...
                        : st === "skip"
                          ...
                          : C.card2,
                    border: `1.5px solid ${
                      st === "ok" ? C.accent + "44" : st === "skip" ? C.red + "33" : C.border
                    }`,
                    borderRadius: 16,
                    padding: "14px 16px",
                    marginBottom: 10,
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <button
                      onClick={() => toggleFood(mealModal.id, f.id, "ok")}
                      style={{
                        background: st === "ok" ? C.accent : `${C.accent}18`,
                        border: `1.5px solid ${C.accent}`,
                        color: st === "ok" ? C.bg : C.accent,
                        borderRadius: 10,
                        padding: "7px 16px",
                        cursor: "pointer",
                        fontWeight: 800,
                    ...
                    <button
                      onClick={() => toggleFood(mealModal.id, f.id, "skip")}
                      style={{
                        background: st === "skip" ? C.red : `${C.red}18`,
                        border: `1.5px solid ${C.red}`,
                        color: st === "skip" ? "#fff" : C.red,
                        borderRadius: 10,
                        padding: "7px 12px",
                        cursor: "pointer",
                        fontWeight: 800,
      ...
      {measureModal && (
        ...
        >
          ...
          >
            ...
            <Btn onClick={addMeasurement} color={C.purple} full style={{ marginTop: 24 }}>
              Salvar Avaliação
            </Btn>
          </div>
        </div>
      ...
      <div style={{ padding: "52px 24px 20px", background: `linear-gradient(180deg,#0f1520 0%,${C.bg} 100%)` }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            ...
            <div style={{ fontFamily: "'Lora',serif", fontSize: 26, fontWeight: 700, lineHeight: 1.2 }}>
              Olá, {(profile?.name || "Aluno").split(" ")[0]}!
            </div>
          ...
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontSize: 34, fontWeight: 900, color: progress >= 80 ? C.accent : C.text }}>
              {progress}%
            </div>
          </div>
        </div>
      ...
      <div style={{ display: "flex", gap: 8, padding: "0 20px 16px", overflowX: "auto" }}>
        {[["hoje", "🍽️", "Hoje"], ["diario", "📓", "Diário"], ["medidas", "📊", "Medidas"], ["config", "⚙️", "Config"]].map(([k, ic, lb]) => (
          <button
            ...
            onClick={() => setView(k)}
            style={{
              background: view === k ? `${C.accent}18` : "transparent",
        ...
        <>
          {view === "hoje" && (
            <div style={{ padding: "0 20px 120px" }}>
              {!notifGranted && !notifDismissed && typeof Notification !== "undefined" && Notification.permission !== "denied" && (
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: C.muted }}>Alertas mesmo com o app fechado.</div>
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <button
                      onClick={async () => {
              ...
              {nutriNotes.length > 0 && nutriNotes[0].note_date === tk && (
                ...
                >
                  <div style={{ fontSize: 12, color: C.purple, fontWeight: 700, marginBottom: 6 }}>
              ...
              >
                <div style={{ fontSize: 13, fontWeight: 700, color: C.sub, marginBottom: 12 }}>
                  Como você está hoje?
                </div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {HUMOR_OPTS.map(h => (
                    <button
                      key={h.v}
                      onClick={async () => {
                        const nl = { ...todayLog, humor: h.v === todayLog.humor ? null : h.v };
                        await persistLog(nl);
                      }}
                      style={{
                        background: todayLog.humor === h.v ? `${h.c}22` : "transparent",
                        border: `1.5px solid ${todayLog.humor === h.v ? h.c : C.border}`,
                        borderRadius: 100,
                        padding: "6px 12px",
                        color: todayLog.humor === h.v ? h.c : C.muted,
                        fontSize: 12,
                        fontWeight: 700,
                        cursor: "pointer",
                        fontFamily: "inherit"
              ...
              <WaterTracker
                ...
                onChange={async n => {
                  await persistLog({ ...todayLog, water: n });
                }}
              />
              ...
              {todayMeals.length === 0 ? (
                <div style={{ textAlign: "center", padding: "50px 20px" }}>
                  ...
                  <div style={{ color: C.muted, marginBottom: 20, lineHeight: 1.6, fontSize: 14 }}>
                    {nutri
                      ? "Aguarde sua nutricionista configurar o cardápio."
                      : "Configure seu cardápio na aba Config."}
                  </div>
                  {!nutri && (
                    <Btn onClick={() => setView("config")} color={C.accent}>
                      Configurar Cardápio →
                    </Btn>
                  )}
                </div>
                ...
                todayMeals.map(meal => {
                  ...
                  return (
                    <div
                      key={meal.id}
                      onClick={() => setMealModal(meal)}
                      style={{
                        background: C.card,
                        border: `1.5px solid ${eaten === total && total > 0 ? C.accent + "55" : C.border}`,
                        borderRadius: 20,
                        padding: "18px 20px",
                        marginBottom: 14,
                        cursor: "pointer"
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ fontSize: 13, color: C.muted }}>
                            {meal.time} · {total} item{total !== 1 ? "s" : ""}
                          </div>
                        </div>
                      ...
                      {todayLog[meal.id]?.photo && (
                        <img
                          src={todayLog[meal.id].photo}
                          alt=""
                          style={{
                            width: "100%",
                            borderRadius: 12,
                      ...
                      <div style={{ marginTop: 12, background: C.card2, borderRadius: 100, height: 3, overflow: "hidden" }}>
                        <div
                          style={{
                            height: "100%",
                            width: `${total ? ((eaten + skipped) / total) * 100 : 0}%`,
                            background: skipped > 0 ? `linear-gradient(90deg,${C.accent},${C.red})` : C.accent,
                            borderRadius: 100
                          }}
                        />
                      </div>
          ...
          {view === "diario" && (
            <div style={{ padding: "0 20px 120px" }}>
              ...
              >
                <div style={{ fontSize: 13, fontWeight: 700, color: C.purple, marginBottom: 12 }}>
                  ✍️ Como foi hoje?
                </div>
                <textarea
                  value={diaryText}
                  onChange={e => setDiaryText(e.target.value)}
                  placeholder="Como se sentiu? O que comeu diferente? Registre sua jornada…"
                  style={{
                    width: "100%",
                    background: C.card2,
                    border: `1.5px solid ${C.border}`,
                ...
                />
                <Btn onClick={saveDiary} color={C.purple} full style={{ marginTop: 12 }}>
                  Salvar ✓
                </Btn>
              </div>
              ...
              {Object.entries(diary)
                ...
                .map(([date, dayData]) => (
                  <div key={date}>
                    ...
                    {dayData.progressPhoto && (
                      <img
                        src={dayData.progressPhoto}
                        alt=""
                        style={{
                          width: "100%",
                          borderRadius: 14,
          ...
          {view === "medidas" && (
            <div style={{ padding: "0 20px 120px" }}>
              {imcVal && (
                <div
                  style={{
                    background: `linear-gradient(135deg,${C.purple}20,${C.accent}15)`,
                    border: `1.5px solid ${C.purple}44`,
                    borderRadius: 20,
                    padding: 22,
                    marginBottom: 16,
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: 44, fontWeight: 900, marginTop: 4 }}>{imcVal}</div>
                  </div>
                  <div
                    style={{
                      textAlign: "right",
              ...
              {waistData.length >= 2 && (
                ...
                >
                  <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 8 }}>
                    〰️ Evolução da Cintura
                  </div>
                  <MiniChart data={waistData} color={C.purple} height={70} />
                </div>
              )}

              <Btn onClick={() => setMeasureModal(true)} color={C.purple} full style={{ marginBottom: 24 }}>
                      ...
                      val ? (
                        ...
                        >
                          ...
                          <div style={{ fontSize: 17, fontWeight: 800 }}>
                            {val}
                            <span style={{ fontSize: 11, color: C.muted }}> {u}</span>
                          </div>
                        </div>
                      ) : null
                    )}
              ...
              >
                <div style={{ display: "flex", gap: 14, alignItems: "center", marginBottom: 14 }}>
                  ...
                  <div>
                    ...
                    <div style={{ color: C.muted, fontSize: 13 }}>{profile.email}</div>
                  </div>
                </div>
              ...
              {!nutri && (
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <div style={{ fontWeight: 800, fontSize: 16 }}>🥗 Meu Cardápio</div>
                      <div style={{ color: C.muted, fontSize: 13, marginTop: 2 }}>
                        {Object.keys(weekDiet).length} configuração(ões)
                      </div>
                    </div>
